...
forval gender=0/1 {
	...
	drop if missing(wagewk)
	keep if female==`gender' 
	gen r90=.
	gen r50=.
	gen r10=.
	forval i=1964(1)2020 {
		summ wagewk if year==`i' [aw=wgt_hrs], detail
...
* Figure 1: Overall Male 90/50 wage inequality
...
twoway ///
	...
	saving("$figures/Figure3/Figure3_panelA_replication.gph",replace)
graph export "$figures/Figure3/Figure3_panelA_replication.pdf", replace

* Figure 2: Overall Female 90/50 wage inequality
use "$data_out/inequality_percentiles.dta", clear
twoway ///
	...
	graphregion(color(white)) ///
	plotregion(lcolor(black) lwidth(thin)) ///
	saving("$figures/Figure3/Figure3_panelB_replication.gph",replace)
graph export "$figures/Figure3/Figure3_panelB_replication.pdf", replace


* Figure 3: Overall Male 50/10 wage inequality
use "$data_out/inequality_percentiles.dta", clear
twoway ///
	connected ineq_cps_50100 ineq_morg_50100 year ///
	, lwidth(medium medium) xlabel(1963(6)2019, labsize(small)) ///
	ylabel(0.3(.05).9, nogrid labsize(small)) ///
	msymbol(Oh Dh) msize(vsmall vsmall) mlwidth(medium medium) ///
	lpattern(solid dash) ///
	...
	graphregion(color(white)) ///
	plotregion(lcolor(black) lwidth(thin)) ///
	saving("$figures/Figure3/Figure3_panelC_replication.gph",replace)
graph export "$figures/Figure3/Figure3_panelC_replication.pdf", replace

* Figure 4: Overall Female 50/10 wage inequality
use "$data_out/inequality_percentiles.dta", clear
twoway ///
	connected ineq_cps_50101 ineq_morg_50101 year ///
	, lwidth(medium mnedium) xlabel(1963(6)2019, labsize(small)) ///
	ylabel(0.3(.05).9, nogrid labsize(small)) ///
	msymbol(Oh Dh) msize(vsmall vsmall) mlwidth(medium medium) ///
	lpattern(solid dash) ///
	ytitle("Log 50/10 wage ratio", size(small)) ///
	...
	graphregion(color(white)) ///
	plotregion(lcolor(black) lwidth(thin)) ///
	saving("$figures/Figure3/Figure3_panelD_replication.gph",replace)
graph export "$figures/Figure3/Figure3_panelD_replication.pdf", replace
