/*==============================================================================
						Cleaning -- CPS ASEC years 1976-1978							
==============================================================================*/	
cap log close 
clear all 
set more off
...
2) indicator for fulltime worker
...
/*------------------------------------------------------------------------------
	Assembling 1976-1978 data
------------------------------------------------------------------------------*/
use "$data_out/ASEC_1976_1978_hrswks.dta", clear

* create variable for age last year (agely): age=71 for age>=71
...
* create flag if household main industry last year is private sector
gen phh=indly==769

* create wage-worker and self-employed variables
gen wageworker=0
replace wageworker=1 if classwly==22 | classwly==25 | classwly==27 | classwly==28
gen selfemp=0
replace selfemp=1 if classwly==13 | classwly==14
...
CPS year. This is accounted for in the file: deflator_2000_pce.do */
merge m:1 year using "$data_out/deflator_2000_pce.dta", keepusing(deflator_2000* pce*) keep(matched) nogen
