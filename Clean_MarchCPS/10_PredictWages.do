...
* keep earnings sample ages 16-64 and experience less than 49 years
...
* summary stats
summ deflator
summ winc_ws female if ftfy [aw=wgt]
for any edhsd edhsg edsmc edclg edgtc: summ winc_ws if X==1 [aw=wgt]
summ lnwinc tchrwg tcwkwg bchrwg bcwkwg bchrwgkm bcwkwgkm female if ftfy [aw=wgt]
summ lnhinc ftfy pt female tchrwg tcwkwg bchrwg bcwkwg bchrwgkm bcwkwgkm [aw=wgt*weeks_lastyear]
...
foreach ed in edhsd edhsg edsmc edclg edgtc {
	...
	* first make all covariates equal to zero
...
* make 25 predictions based on the 5 education and 5 experience categories
foreach ed in edhsd edhsg edsmc edclg edgtc {
    ...
    forval exp=5(10)45 {
      ...
      * interact education with experience variables defined above 
      ...
      if "`ed'"=="edgtc" { 
        ...
        keep e1* e2* e3* e4* exp* ed* deflator black female other pt year mw so we
        predict plnhrw in 1
        sum plnhrw
        keep if _n==1
...
gsort -female
reg lnhinc edhsd edsmc edclg edgtc exp1 exp2 exp3 exp4 e1* e2* e3* e4* pt ///
	black other mw so we [aw=wgt*weeks_lastyear] if female & !tchrwg & !bchrwgkm

* predict values
assert female==1 in 1

* make 25 predictions based upon the the 5 education and 5 experience categories
foreach ed in edhsd edhsg edsmc edclg edgtc {
...
* deleting not needed data files
